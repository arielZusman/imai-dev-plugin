...
claude --plugin-dir /path/to/implementation-planner
```
...
/implementation-planner:generate-plan my-feature

# 4. Validate against codebase patterns
/implementation-planner:validate-plan

# 5. Execute with orchestration
/implementation-planner:execute-plan my-feature

# 6. When rotation is recommended (after 4 tasks or 30 min)
# Clear session, then:
/implementation-planner:resume-plan my-feature
```
...
|---------|-------------|-------|
| `prime` | Prime session with git context | `/implementation-planner:prime` |
| `generate-plan` | Enrich plan for standalone execution | `/implementation-planner:generate-plan [plan-path]` |
| `validate-plan` | Validate plan against codebase | `/implementation-planner:validate-plan [plan-path]` |
| `execute-plan` | Execute plan with orchestration | `/implementation-planner:execute-plan <plan> [tasks]` |
| `checkpoint` | Save execution checkpoint | `/implementation-planner:checkpoint <plan> <task> <status>` |
| `resume-plan` | Resume plan from checkpoint | `/implementation-planner:resume-plan <plan> [task-id]` |
